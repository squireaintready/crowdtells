...
const SYSTEM = [
  ...
  'Follow the CATEGORY PLAYBOOK for what the lead must cover — SPORTS: the two teams, the matchup and stakes (standings, rivalry, streak, series, title), recent form if known, and when/where it is played. PRICE (crypto/stocks/levels): the asset and the forces moving it (a catalyst, a level it is testing). POLITICS: the candidates, offices, parties, the contest and timeline. DECISION (ruling/launch/nomination/award): the decision-maker, what is being decided, and the deadline or trigger.',
...
export function buildUser(ctx: MarketContext, headlines: Headline[], today: string): string {
  ...
  return [
    ...
    '- "dek": one standfirst sentence of 28 words or fewer under the headline — what is happening and what is at stake in the real world. Lead with the event. No tokens, no figures, no mention of the market, odds, or crowd.',
